    ...
    <section className="py-16 md:py-32 px-6 bg-soft-lilac/30" id="path">
      <div className="max-w-4xl mx-auto">
        <div className="mb-12 md:mb-20">
          <span className="text-eggplant font-bold tracking-[0.3em] uppercase text-[10px]">
        ...
        <div className="space-y-0 divide-y divide-charcoal/10">
          {t.experienceData.map((exp, idx) => (
            <div
              key={idx}
              className="py-8 md:py-12 first:pt-0 last:pb-0 flex flex-col md:flex-row md:items-start justify-between group"
            >
                ...
                {exp.bulletPoints && exp.bulletPoints.length > 0 && (
                  <ul className="space-y-2 mb-6">
                    {exp.bulletPoints.map((point, pointIdx) => (
                      <li
                        key={pointIdx}
                        className="text-charcoal/60 text-sm leading-relaxed flex items-start"
                      >
